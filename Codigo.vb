...
for each PedCod
  where PedCod = &PedCod
    ...
    &PedVlrM3Fret   = PedVlrM3Fret 
    &PedVol         = PedVol
    &PedVeicCod     = PedVeicCod
...
//            if PdiPrdControlaLote = "S"
//              if &PedTipTrnNOP = 2 or &PedTipTrnNOP = 10 or &PedTipTrnNOP = 11 or &PedTipTrnNOP = 8 
//              
//                do'verlote'
//                
//                if &flag3 = 'N'
//                  Msg('ATENÇÃO, os lotes do produto '+PdiPrdCod.ToString().Trim()+' não foram cadastrados no pedido !!!')
//                  return
//                endif
//                
//                if &flag3 = 'S' and &PdiQtd3 <> PdiQtd
//                  Msg('ATENÇÃO, a quantidade informada nos lotes do produto '+PdiPrdCod.ToString().Trim()+' não confere com a quantidade informada no pedido !!!')
//                  return
//                endif
//              
//              endif
...
//            &SdtPdi.PdiOrdCompraSeq = PdiOrdCompraSeq
            ...
            &SdtPdi.PdiPrdControlaLote = PdiPrdControlaLote
...
for each PedCod
    ...
    for &sdtpdi in &ColSdtPdi
        if &SdtPdi.Seq = 1
           ...
           &SdtNfProItem.NfiControlaLote = &SdtPdi.PdiPrdControlaLote
                ...
                for &SdtPdi in &ColSdtPdi
                    if &SdtPdi.Seq = 2
                       ...
                       if (&PedMovStq = 1 or (&SdtPdi.PdiCfopMovStq = 'S' and &EmpTpMovStqCcr = 'C') ) and &CreTp = 'Q' // Controla Movimento de Estoque e Contas a Receber pelo Pedido ou CFOP
                           ...
                           PGravaStq2.Call(&Logon,&PdiPrdCod,&PdiQtd,&ES,&PdiVlrUnt,&obs,&PedCod,&MovSeq,'',0,'V')
...
sub'obs'
     ...
     If &Logon.EmpClienteNa12 = 19 //Uguettolar

 
        If &PedVlrM3Fret > 0

           &ValorFrete = &PedVlrM3Fret * &SdtNf.NfsCubagem
           &VlrICMS = (&ValorFrete*(12/100))                //12% de ICMS
    
           &SdtNf.NfsInfCmp += 'FRETE EFETUADO POR TRANSPOTADOR AUTONOMO, VALOR DO FRETE R$ ' +  Trim(ToFormattedString(&ValorFrete)) + ' ALIQUOTA ICMS 12% R$ ' + Trim(ToFormattedString(&VlrICMS)) + '. RESP. PELO PGTO: REMETENTE / '
     ...
     if not null(&PedRedespCod)    
         for each
             where TrpCod = &PedRedespCod    
                if TrpTp = 'J'
                  &CnpjCpf = TrpCnpj.Trim()
                else
                  &CnpjCpf = TrpCpf.Trim()
                endif

                &contatoTrp = ' Contato: ' + TrpCel.Trim()

                if not Null(TrpFone)
                  &contatoTrp += TrpFone.Trim()
                endif
                ...
                If &Logon.EmpClienteNa12 = 19 //Uguettolar
                    &SdtNf.NfsInfCmp += 'Redespacho: '+TrpNom.Trim()+' Endereço:'+TrpEnd.Trim()+','+TrpEndNum.Trim()+' - '+TrpEndBai.Trim()+'  '+TrpCidNom.Trim()+' - '+TrpUFCod.Trim() + ' CNPJ/CPF:' + &CnpjCpf.Trim() + ' ' + &contatoTrp.Trim() + ' ' + &ie.Trim() + '  RESP. PELO PGTO. DO REDESPACHO: DESTINATARIO. / ' 
                Else
                    &SdtNf.NfsInfCmp += 'Redespacho: '+TrpNom.Trim()+' Endereço:'+TrpEnd.Trim()+','+TrpEndNum.Trim()+' - '+TrpEndBai.Trim()+'  '+TrpCidNom.Trim()+' - '+TrpUFCod.Trim() + ' CNPJ/CPF:' + &CnpjCpf.Trim() + ' ' + &contatoTrp.Trim() + ' ' + &ie.Trim() + ' / ' 
                EndIf
         endfor    
     endif 
...
//     endif
     ...
     if &EmpCrt <> '3'
      ...
      if not null(&CliObsNF)
           &SdtNf.NfsInfCmp += '\ ' + 'Obs. Cliente: ' + trim(&CliObsNF) 
      endif
        ...
        where PedCod = &PedCod
            ...
            if &CreTp = 'Q' // QUANTIDADE
                   ...
                   do case
                     ...
                     case &EmpRoundNF = 4
                       &Valor = round(PdiVlrUnt * &CreVlr / 100, 4)
                     case &EmpRoundNF = 5
                       &Valor = round(PdiVlrUnt * &CreVlr / 100, 5)
                     case &EmpRoundNF = 6
                       &Valor = round(PdiVlrUnt * &CreVlr / 100, 6) 
                     otherwise     // &EmpRoundNF = 2 or null(&EmpRoundNF)
                       &Valor = round(PdiVlrUnt * &CreVlr / 100, 2)
                   endcase
